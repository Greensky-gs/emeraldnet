...
export class EmeraldUsersManager {
	...
	public createUser({
		...
		this.database(
			`INSERT INTO ${
				Tables.Users
			} ( id, login, password, perm, allowedOn ) VALUES ( "${id}", "${sqllise(
				login,
			)}", "${this.hash(password)}", "${resolvePerm(perm)}", "[]" )`,
		);
		return this.cache[id];
	}
	...
	public updateUser(update: updateUser) {
		...
		this.database(
			`UPDATE ${Tables.Users} SET ${Object.entries(update)
				.map(([k, v]) => `${k}="${sqllise(v.toString())}"`)
	...
	public allow(userId: string, ip: string) {
		...
		this.database(
			`UPDATE ${Tables.Users} SET allowedOn="${sqllise(
				JSON.stringify(user.allowedOn),
			)}" WHERE id="${user.id}"`,
		);
	}
